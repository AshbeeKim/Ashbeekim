...
class EnvInfos:
    '''
    !pip install psutill, gputill, humanize
    import os, sys, psutill, humanize ;import GPUtill as GPU ;import numpy as np ;import pandas as pd
    # 기본 단위 : Byte
    # 1 byte = 8 bit
    # 1 KB = 1024 bytes             -> KiB(키비바이트) = 2 ** 10
    # 1 MB = 1024 ** 2 bytes
    # 1 GB = 1024 ** 3 bytes
    # 1 TB = 1024 ** 4 bytes
    # PB -> EB -> ZB -> YB
